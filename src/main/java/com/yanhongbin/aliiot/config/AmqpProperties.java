...
import com.yanhongbin.aliiot.config.enumerate.SignMethod;
 ...
 */
@ConfigurationProperties("spring.yanhongbin.ali.iot.amqp")
@Component
@Scope("singleton")
@Data
    ...
    public void afterPropertiesSet() throws Exception {
        ...
        if (connectConfig.getType() == null|| connectConfig.getType() == ConnectSetting.AMQP) {
            ...
            if (consumerGroupMessageConfig == null || consumerGroupMessageConfig.length == 0) {
                throw new PropertiesNotFoundException("无法找到 spring.yanhongbin.ali.iot.amqp.consumerGroupMessageConfig 配置信息");
            }
        }
        if (signMethod == null) {
            // 默认使用 hmacsha1 生成签名
